...
function renderFailureCard(message, retry) {
  const node = document.createElement("div");
  node.className = "observable-render-failure";
  ...
  node.style.border = "1px dashed var(--theme-foreground-faint)";
...
/**
 * Wrap an embed's render function so that a throw, rejection, or stall shows a
 * retryable card in its place instead of Observable's red error text, and one
    ...
    try {
      ...
      const message =
        error?.message === "timed out"
          ? "This chart is taking too long to load."
