...
import {chart as createChart} from "./echart.459e0195.js";
...
const MARK_RENDERERS = {
  dot: (rows, options, context) => {
    ...
    if (options.large === true && rows.length > 2000) {
      ...
      // constant, since large mode has no per-point size either.
      const dataByColor = new Map();
      for (const row of rows) {
        const x = getValue(row, options.x);
        const y = getValue(row, options.y);
        if (x == null || y == null) continue;
        const color = scaleManager.getColor("fill", getValue(row, colorAccessor));
        let data = dataByColor.get(color);
        if (!data) dataByColor.set(color, (data = []));
        data.push([x, y, rowRef(row)]);
      }
      const symbolSize = rows.length > 0 ? sizeFor(rows[0]) : 6;
      return {
        series: Array.from(dataByColor, ([color, data]) => ({
          type: "scatter",
          ...axisIndex,
          data,
          symbolSize,
...
function buildMarkSeries(mark, context, axisIndex = {xAxisIndex: 0, yAxisIndex: 0}, facetValue = null) {
  ...
  const markContext = {
    ...
    tooltipFormatter: (params) => {
      const rowIdx = Array.isArray(params.data) ? params.data[2] : params.data?.__rowIdx;
      const row = context.rowIndex[rowIdx];
...
function buildOption(config, marks) {
  ...
  if (facetMode) {
    ...
    const hasDenseScatter = option.series.some(
      (series) => series.type === "scatter" && (series.large === true || (series.data?.length || 0) > 2000)
    );
...
function collectSeriesPoints(option) {
  ...
  for (const series of seriesList) {
    ...
    if (type !== "line" && type !== "scatter" && type !== "bar") continue;
...
export function plot(config = {}) {
  ...
  const host = createChart(option, {
    ...
    onReady(instance) {
      ...
      if (typeof config.onReady === "function") config.onReady(instance);
    }
  });
