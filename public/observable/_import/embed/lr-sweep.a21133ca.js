...
function debounce(fn, delayMs = 90) {
  let timeoutId = null;
  return (...args) => {
...
export async function renderOptimizerSweepClusters(options = {}) {
  ...
  const clusterBestEvalRows = pointRows.filter((row) => Number.isFinite(row.eval_best_loss) && row.eval_best_loss > 0);
  const clusterBestEvalMin = d3.min(clusterBestEvalRows, (row) => row.eval_best_loss) ?? 1e-3;
  const clusterBestEvalMax = d3.max(clusterBestEvalRows, (row) => row.eval_best_loss) ?? 10;
  const clusterBestEvalStep = Math.max(1e-6, (clusterBestEvalMax - clusterBestEvalMin) / 300);
  const bestEvalCutoffControl = rangeControl(
    "Best eval-loss cutoff (main clusters)",
    clusterBestEvalMin,
    clusterBestEvalMax,
    clusterBestEvalStep,
    Number.isFinite(Number(options.clusterBestEvalCutoff))
      ? Math.max(clusterBestEvalMin, Math.min(clusterBestEvalMax, Number(options.clusterBestEvalCutoff)))
      : clusterBestEvalMax
  );
  ...
  controls.append(kControl.node, bestEvalCutoffControl.node);
  subsetControlHost.append(el("strong", "Subset controls"), kSubsetControl.node);

  function refresh() {
    const bestEvalCutoff = Math.max(
      clusterBestEvalMin,
      Math.min(clusterBestEvalMax, Number(bestEvalCutoffControl.input.value) || clusterBestEvalMax)
    );
    bestEvalCutoffControl.input.value = String(bestEvalCutoff);
    bestEvalCutoffControl.output.textContent = bestEvalCutoff.toFixed(6);

    const mainFilteredRows = pointRows.filter(
      (row) => Number.isFinite(row.eval_best_loss) && row.eval_best_loss > 0 && row.eval_best_loss <= bestEvalCutoff
    );
    ...
    if (mainFilteredCount >= 2) {
      ...
      const mainBestEvalLogMin = d3.min(mainFilteredRows, (row) => Math.log10(row.eval_best_loss)) ?? Math.log10(clusterBestEvalMin);
      const mainBestEvalLogMax = Math.log10(bestEvalCutoff);
    ...
    const parallelTickRows = parallelAxisSpecs.flatMap((axis, axisIndex) =>
      ...
      chartHost.appendChild(
        ...
        marks: [
          ...
          Plot.dot(parallelRows, {
            ...
            tip: true,
            title: (d) =>
              `${d.cluster_label}\nRun # ${Number.isFinite(d.run_number) ? d.run_number : "n/a"} (${d.run_name})\nβ2 ${Number(
                d.optimizer_beta2
      ...
      chartHost.appendChild(
        emptyState(`Need at least two runs at or below best eval-loss cutoff ${bestEvalCutoff.toFixed(6)} for main clusters.`)
      );
    }
      ...
      subsetChartHost.appendChild(
        Plot.plot({
          ...
          marks: [
            ...
            Plot.dot(subsetParallelRows, {
              x: "axis_index",
              y: "axis_value_norm",
              fill: "subset_cluster_label",
              r: 2.8,
              opacity: 0.9,
              tip: true,
              title: (d) =>
    ...
    summaryHost.textContent =
      `Main clusters use normalized [β2, log10(weight decay), log10(max LR)] for runs with best eval loss <= ${bestEvalCutoff.toFixed(
        6
  ...
  attachInputRefresh(bestEvalCutoffControl.input, refresh);
...
export async function renderOptimizerSweepEvalLoss(options = {}) {
  ...
  const bestEvalLossValues = Array.from(bestEvalLossByRun.values()).filter((value) => Number.isFinite(value) && value > 0);
  const bestEvalLossMin = d3.min(bestEvalLossValues) ?? NaN;
  const bestEvalLossMax = d3.max(bestEvalLossValues) ?? NaN;
  let bestEvalCutoffControl = null;
  if (Number.isFinite(bestEvalLossMin) && Number.isFinite(bestEvalLossMax)) {
    const cutoffStep =
      bestEvalLossMax > bestEvalLossMin ? Math.max(1e-6, Number(((bestEvalLossMax - bestEvalLossMin) / 500).toPrecision(6))) : 1e-6;
    bestEvalCutoffControl = rangeControl(
      "Max best-eval-loss cutoff",
      bestEvalLossMin,
      bestEvalLossMax,
      cutoffStep,
      Number.isFinite(Number(options.bestEvalLossCutoff))
        ? Math.max(bestEvalLossMin, Math.min(bestEvalLossMax, Number(options.bestEvalLossCutoff)))
        : bestEvalLossMax
    );
    controlsRow.appendChild(bestEvalCutoffControl.node);
  }
  ...
  function renderMuRegressionChart() {
    ...
    const xValues = regressionRows
      .map((row) => row.iteration)
      .filter((value) => Number.isFinite(value))
      .sort(d3.ascending);
    const yValues = regressionRows
      .map((row) => row.value)
      .filter((value) => Number.isFinite(value))
      .sort(d3.ascending);
    ...
    const xStep = Number.isFinite(xMin) && Number.isFinite(xMax) && xMax > xMin ? Math.max(1, Math.round((xMax - xMin) / 400)) : 1;
    const yStep =
      Number.isFinite(yMin) && Number.isFinite(yMax) && yMax > yMin
        ? Number(((yMax - yMin) / 500).toPrecision(6))
        : Math.max(1e-6, Math.abs(yMax || 1) * 1e-3);
    const xCutoffControl = rangeControl("Max iteration cutoff", xMin, xMax, xStep, xMax);
    const yCutoffControl = rangeControl("Max coefficient cutoff", yMin, yMax, yStep, yMax);
    controls.append(xCutoffControl.node, yCutoffControl.node);
    muRegressionHost.appendChild(controls);

    const chartHost = el("div");
    muRegressionHost.appendChild(chartHost);

    function refreshRegressionChart() {
      const xUpper = Math.max(xMin, Math.min(xMax, Number(xCutoffControl.input.value) || xMax));
      const yUpper = Math.max(yMin, Math.min(yMax, Number(yCutoffControl.input.value) || yMax));
      xCutoffControl.input.value = String(xUpper);
      yCutoffControl.input.value = String(yUpper);
      xCutoffControl.output.textContent = Number.isFinite(xUpper) ? Math.round(xUpper).toLocaleString("en-US") : "n/a";
      yCutoffControl.output.textContent = Number.isFinite(yUpper) ? Number(yUpper).toFixed(6) : "n/a";
      const filteredRows = regressionRows.filter(
        (row) =>
          (!Number.isFinite(xUpper) || row.iteration <= xUpper) &&
          (!Number.isFinite(yUpper) || row.value <= yUpper)
      );
      const ciRows = filteredRows.filter(
        (row) => Number.isFinite(row.ci_low) && Number.isFinite(row.ci_high) && row.ci_low <= row.ci_high
      );

      clearNode(chartHost);
      if (filteredRows.length === 0) {
        chartHost.appendChild(emptyState("No regression coefficient points remain after axis-end trimming."));
        return;
      }
      ...
      chartHost.appendChild(
        Plot.plot({
          ...
          marks: [
            ...
            Plot.lineY(filteredRows, {x: "iteration", y: "value", stroke: "series", z: "series"}),
            Plot.dot(filteredRows, {
              ...
              opacity: 0.65,
              title: (d) =>
                `${d.series}\nIteration ${Math.round(d.iteration)}\nCoefficient ${Number(d.value).toFixed(6)}\nRuns ${Math.round(d.sample_size)}`,
              tip: true
            })
          ]
        })
    ...
    attachInputRefresh(xCutoffControl.input, refreshRegressionChart);
    attachInputRefresh(yCutoffControl.input, refreshRegressionChart);
    refreshRegressionChart();
  }
  ...
  function renderMuQuadrantAnalysis() {
    ...
    const evalStep = Math.max(1e-4, Number(((fullEvalMax - fullEvalMin) / 300).toPrecision(4)));
    const mStep = Math.max(1e-4, Number(((fullMMax - fullMMin) / 300).toPrecision(4)));

    const evalCutoffControl = rangeControl(
      "Max eval loss @125",
      Number(fullEvalMin.toFixed(4)),
      Number(fullEvalMax.toFixed(4)),
      evalStep,
      Number(fullEvalMax.toFixed(4))
    );
    const mMaxControl = rangeControl(
      "Max M @125",
      Number(fullMMin.toFixed(4)),
      Number(fullMMax.toFixed(4)),
      mStep,
      Number(fullMMax.toFixed(4))
    );

    for (const ctrl of [evalCutoffControl, mMaxControl]) {
      ctrl.node.style.minWidth = "220px";
      ctrl.node.style.alignItems = "center";
      ctrl.node.style.justifyContent = "space-between";
      ctrl.input.style.flex = "1";
      ctrl.input.style.minWidth = "0";
      ctrl.output.style.minWidth = "7ch";
      ctrl.output.style.textAlign = "right";
      ctrl.output.style.fontVariantNumeric = "tabular-nums";
    }
    const controlsRow = el("div");
    controlsRow.style.display = "grid";
    controlsRow.style.gridTemplateColumns = "repeat(auto-fit, minmax(220px, 1fr))";
    controlsRow.style.gap = "0.5rem 1rem";
    controlsRow.append(evalCutoffControl.node, mMaxControl.node);
    muQuadrantHost.appendChild(controlsRow);
    ...
    function refreshQuadrant() {
      const maxEval = Math.min(fullEvalMax, Math.max(fullEvalMin, Number(evalCutoffControl.input.value) || fullEvalMax));
      const minM = fullMMin;
      let maxM = Math.min(fullMMax, Math.max(fullMMin, Number(mMaxControl.input.value) || fullMMax));
      if (minM > maxM) maxM = minM;
      mMaxControl.input.value = String(maxM);

      evalCutoffControl.output.textContent = Number(maxEval).toFixed(4);
      mMaxControl.output.textContent = Number(maxM).toFixed(4);

      const filtered = allQuadrantRows.filter(
        (row) => row.eval_loss <= maxEval && row.sweep_metric_m >= minM && row.sweep_metric_m <= maxM
      );

      summaryLine.textContent =
        `${filtered.length} of ${allQuadrantRows.length} runs shown` +
        (filtered.length < allQuadrantRows.length ? ` (${allQuadrantRows.length - filtered.length} filtered out)` : "") +
        ".";
      ...
      if (filtered.length < 4) {
        chartHost.appendChild(emptyState("Too few runs remain after filtering to draw the quadrant chart."));
        return;
      }

      const medianM = d3.median(filtered, (row) => row.sweep_metric_m);
      const medianEval = d3.median(filtered, (row) => row.eval_loss);

      for (const row of filtered) {
        const goodM = row.sweep_metric_m <= medianM;
        const goodEval = row.eval_loss <= medianEval;
        if (goodEval && goodM) row.quadrant = "Good Eval / Good M";
        else if (!goodEval && goodM) row.quadrant = "Bad Eval / Good M \u2605";
        else if (goodEval && !goodM) row.quadrant = "Good Eval / Bad M";
        else row.quadrant = "Bad Eval / Bad M";
      }

      const mMin = d3.min(filtered, (row) => row.sweep_metric_m);
      const mMax = d3.max(filtered, (row) => row.sweep_metric_m);
      const eMin = d3.min(filtered, (row) => row.eval_loss);
      const eMax = d3.max(filtered, (row) => row.eval_loss);

      const labelData = [
        {x: (eMin + medianEval) / 2, y: (mMin + medianM) / 2, text: "Good Eval\nGood M"},
        {x: (medianEval + eMax) / 2, y: (mMin + medianM) / 2, text: "Bad Eval\nGood M \u2605"},
        {x: (eMin + medianEval) / 2, y: (medianM + mMax) / 2, text: "Good Eval\nBad M"},
        {x: (medianEval + eMax) / 2, y: (medianM + mMax) / 2, text: "Bad Eval\nBad M"}
      ];
      ...
      chartHost.appendChild(
        Plot.plot({
          ...
          marks: [
            Plot.ruleX([medianEval], {stroke: "currentColor", strokeDasharray: "6 4", strokeOpacity: 0.4}),
            Plot.ruleY([medianM], {stroke: "currentColor", strokeDasharray: "6 4", strokeOpacity: 0.4}),
            Plot.text(labelData, {
              x: "x",
              y: "y",
              text: "text",
            ...
            Plot.dot(filtered, {
              x: "eval_loss",
              y: "sweep_metric_m",
              fill: "final_iteration",
              r: 4,
              opacity: 0.8,
              tip: true,
              title: (d) =>
                `Run ${d.run_number}: ${d.run_name}\nM @125: ${Number(d.sweep_metric_m).toFixed(4)}` +
      ...
      const quadrantSummary = quadrantOrder.map((q) => {
        const qRows = filtered.filter((row) => row.quadrant === q);
    ...
    attachInputRefresh(evalCutoffControl.input, refreshQuadrant, {delayMs: 150});
    attachInputRefresh(mMaxControl.input, refreshQuadrant, {delayMs: 150});
    refreshQuadrant();
  }
  ...
  function refresh() {
    ...
    let maxBestEvalCutoff = NaN;
    let allowedRuns = null;
    if (bestEvalCutoffControl) {
      maxBestEvalCutoff = Math.max(
        bestEvalLossMin,
        Math.min(bestEvalLossMax, Number(bestEvalCutoffControl.input.value) || bestEvalLossMax)
      );
      bestEvalCutoffControl.input.value = String(maxBestEvalCutoff);
      bestEvalCutoffControl.output.textContent = Number(maxBestEvalCutoff).toFixed(6);
      allowedRuns = new Set(
        Array.from(bestEvalLossByRun.entries())
          .filter(([, best]) => Number.isFinite(best) && best > 0 && best <= maxBestEvalCutoff)
          .map(([runName]) => runName)
      );
    }

    const windowRows = enrichedRows.filter(
      (row) =>
        row.step >= minStep &&
        row.step <= maxStep &&
        (!allowedRuns || allowedRuns.has(row.run_name))
    );
    ...
    const runCountAfterBestEvalFilter = allowedRuns ? allowedRuns.size : runNames.length;
    summaryHost.textContent = `${runNames.length.toLocaleString("en-US")} runs total, ${runCountAfterBestEvalFilter.toLocaleString(
      "en-US"
    )} runs after best-eval filter, ${evalRows.length.toLocaleString(
      "en-US"
      ...
      Number.isFinite(maxBestEvalCutoff) ? `, best eval <= ${Number(maxBestEvalCutoff).toFixed(6)}` : ""
    }.`;
  ...
  attachInputRefresh(excludeFirstControl.input, refresh, {onInput: () => syncExcludeValues("first")});
  attachInputRefresh(excludeLastControl.input, refresh, {onInput: () => syncExcludeValues("last")});
  if (bestEvalCutoffControl) attachInputRefresh(bestEvalCutoffControl.input, refresh);
  refresh();
  return root;
}
  ...
  root.appendChild(
    Plot.plot({
      ...
      marks: [
        ...
        Plot.lineY(runRows, {x: "step", y: "eval_loss", strokeWidth: 2.2}),
        Plot.dot(runRows, {
          x: "step",
          y: "eval_loss",
          r: 2.6,
...
export async function renderLrSweepLrGradScatter(options = {}) {
  ...
  attachInputRefresh(minLrControl.input, refresh);
  attachInputRefresh(maxGradControl.input, refresh);
  refresh();
...
export async function renderLrSweepSummaryTable(options = {}) {
  ...
  if (windowControl) windowControl.onChange(debounce(refreshSummary, 90));
  refreshSummary();
