...
import * as Plot from "../components/echart-plot.823a35ae.js";
import * as d3 from "../../_npm/d3@7.9.0/080cf928.js";
import {readParquet} from "../components/parquet.4291c806.js";
...
const ATTACHMENTS = {
  mps_main: FileAttachment({"name":"../../data/raw/benchmarks/mps_comp_main.parquet","mimeType":undefined,"path":"../../_file/data/raw/benchmarks/mps_comp_main.f7579557.parquet","lastModified":1771874136937,"size":21062}, import.meta.url),
  gpu_main: FileAttachment({"name":"../../data/raw/benchmarks/gpu_comp_main.parquet","mimeType":undefined,"path":"../../_file/data/raw/benchmarks/gpu_comp_main.d82fa693.parquet","lastModified":1771874136918,"size":21863}, import.meta.url),
  mps_history: FileAttachment({"name":"../../data/raw/benchmarks/mps_comp_history.parquet","mimeType":undefined,"path":"../../_file/data/raw/benchmarks/mps_comp_history.8ee202e9.parquet","lastModified":1771874136937,"size":1535334}, import.meta.url),
  gpu_history: FileAttachment({"name":"../../data/raw/benchmarks/gpu_comp_history.parquet","mimeType":undefined,"path":"../../_file/data/raw/benchmarks/gpu_comp_history.e18efbee.parquet","lastModified":1771874136917,"size":1389327}, import.meta.url),
  ...
  catalog: FileAttachment({"name":"../../data/raw/llm-fundamentals/model-config-catalog.json","mimeType":"application/json","path":"../../_file/data/raw/llm-fundamentals/model-config-catalog.556b23f1.json","lastModified":1772246988772,"size":8539}, import.meta.url)
};
...
async function loadMainRows(datasetName, platform, attachment) {
  const table = await readParquet(attachment);
...
// Every history column the row mapping below reads; the files carry 32.
const HISTORY_COLUMNS = [
  "run_name", "config.run_name", "run_id", "_step", "Loss", "Eval Loss", "Eval/Loss", "Eval Perplexity",
  "Eval/Perplexity", "Throughput/Tokens per sec", "Time/Total step", "Grad/Norm (unclipped)",
  "Grad/Norm (pre-clip)", "Grad/Norm (preclip)", "Grad/Norm (raw)", "Grad/Norm"
];

async function loadHistoryRows(datasetName, platform, attachment) {
  const table = await readParquet(attachment, {columns: HISTORY_COLUMNS});
  return Array.from(table, (row) => {
    const runName = pickString(row, ["run_name", "config.run_name", "run_id"], "unknown-run");
    const runLabel = normalizeRunLabel(runName);
    const modelId = inferModelId(runLabel);
...
export async function loadEmpiricalData() {
  if (!empiricalDataPromise) {
    ...
    empiricalDataPromise.catch(() => {
      empiricalDataPromise = null;
    });
  }
...
function renderExpectedVsActualSection(data, options = {}) {
  ...
  function buildDiagonalScatter(hostEl, rows, xKey, yKey, xLabel, yLabel, emptyMsg, colorBy, scaleOpts = {}) {
    ...
    const xDomain =
      Array.isArray(scaleOpts.domainX) &&
      Number.isFinite(Number(scaleOpts.domainX[0])) &&
      Number.isFinite(Number(scaleOpts.domainX[1])) &&
      Number(scaleOpts.domainX[1]) > Number(scaleOpts.domainX[0]) &&
      (!scaleOpts.logX || Number(scaleOpts.domainX[0]) > 0)
        ? [Number(scaleOpts.domainX[0]), Number(scaleOpts.domainX[1])]
        : undefined;
    const yDomain =
      Array.isArray(scaleOpts.domainY) &&
      Number.isFinite(Number(scaleOpts.domainY[0])) &&
      Number.isFinite(Number(scaleOpts.domainY[1])) &&
      Number(scaleOpts.domainY[1]) > Number(scaleOpts.domainY[0]) &&
      (!scaleOpts.logY || Number(scaleOpts.domainY[0]) > 0)
        ? [Number(scaleOpts.domainY[0]), Number(scaleOpts.domainY[1])]
        : undefined;
    hostEl.appendChild(
      Plot.plot({
        width: 920,
        height: 380,
        marginBottom: 72,
        x: {label: xLabel, grid: true, type: scaleOpts.logX ? "log" : "linear", ...(xDomain ? {domain: xDomain} : {})},
        y: {label: yLabel, grid: true, type: scaleOpts.logY ? "log" : "linear", ...(yDomain ? {domain: yDomain} : {})},
        dataZoom: scaleOpts.zoomY ? Plot.dataZoomXY({right: 10}) : Plot.dataZoomX(),
        color: colorConfig,
        marks: [
...
function renderDeviceComparisonSection(data, options = {}) {
  ...
  const tableHost = card();
  const throughputScaleControl = selectControl(
    "Throughput chart scale",
    [
      {value: "linear", label: "Linear"},
      {value: "log", label: "Log"}
    ],
    options.throughputScale || "linear"
  );
  const controls = card();
  controls.append(throughputScaleControl.node);
  host.append(controls, throughputHost, iterationsHost, speedupHost, performanceHost, tableHost);

  const renderMetricBarChart = (container, chartTitle, yLabel, valueKey, scaleType) => {
    clearNode(container);
    ...
    const numericRows = chartRows.filter((row) => {
      const value = Number(row[valueKey]);
      if (!Number.isFinite(value)) return false;
      if (scaleType === "log") return value > 0;
      return true;
    });
    if (numericRows.length === 0) {
      container.appendChild(emptyState(`No rows available for ${chartTitle.toLowerCase()} on ${scaleType} scale.`));
      return;
    }
    const yMax = d3.max(numericRows, (row) => Number(row[valueKey])) ?? 0;
    const yLinearMax = yMax > 0 ? yMax * 1.05 : 1;
    const yConfig =
      scaleType === "log"
        ? {label: `${yLabel} (log scale)`, type: "log", grid: true}
        : {label: yLabel, grid: true, domain: [0, yLinearMax]};
    ...
    container.appendChild(
      Plot.plot({
        ...
        marginBottom: 72,
        marginTop: 40,
        x: {label: null, domain: modelValues},
        y: yConfig,
        color: {legend: true, label: "Device", scheme: "observable10"},
        marks: [
          ...(scaleType === "linear" ? [Plot.ruleY([0], {stroke: "var(--theme-foreground-faint)"})] : []),
          Plot.barY(numericRows, {
            x: "model_label",
            y: valueKey,
            fill: "platform_label",
            title: (d) =>
  ...
  const refresh = () => {
    ...
    renderMetricBarChart(iterationsHost, "Iterations / sec by model and device", "Iterations / sec", "iterations_per_sec", scaleType);
    ...
    if (speedupRows.length === 0) {
      ...
      const xMax = d3.max(sorted, (row) => Number(row.throughput_speedup)) ?? 1;
      const xUpper = xMax > 0 ? xMax * 1.05 : 1;
      speedupHost.appendChild(
        Plot.plot({
          width: 920,
          height: Math.min(420, 140 + sorted.length * 48),
          marginLeft: 220,
          x: {label: "CUDA / MPS throughput speedup", grid: true, domain: [0, xUpper]},
  ...
  throughputScaleControl.select.addEventListener("change", refresh);
  refresh();
  return host;
}
...
function renderTrainingCurvesSection(data, options = {}) {
  ...
  const seriesOptions = Array.from(
    new Map(curveRows.map((row) => [row.series_key, row.series_label || row.series_key])).entries()
  )
  ...
  function buildMainCurveRows(selectedSeries, curveMetric, channelMode = "both") {
    const output = [];
    ...
    for (const [, rows] of grouped) {
      ...
      for (const row of ordered) {
        const step = Number(row.step);
        if (!Number.isFinite(step)) continue;
        const seriesBase = row.series_label || row.series_key;
  ...
  function buildGradRows(selectedSeries) {
    const output = [];
    const grouped = d3.groups(
      curveRows.filter((row) => selectedSeries.has(row.series_key)),
      (row) => row.series_key
    ...
    for (const [, rows] of grouped) {
      ...
      for (const row of ordered) {
        ...
        output.push({
          ...
          curve_series_label: `${row.series_label || row.series_key} (Pre-clip grad norm)`
        });
      }
    }
    return output.sort((a, b) => d3.ascending(a.step, b.step));
  ...
  const refresh = () => {
    ...
    const mainRows = buildMainCurveRows(selectedSeries, curveMetric, channelMode);
    ...
    const gradRows = buildGradRows(selectedSeries);
    ...
    if (mainRows.length === 0) {
      ...
      mainChartHost.appendChild(
        Plot.plot({
          ...
          dataZoom: Plot.dataZoomX(),
          color: {legend: true, label: "Series"},
          marks: mainMarks
        })
      );
...
export const renderPerfEmpiricalTrainBenchmark = resilientRender("renderPerfEmpiricalTrainBenchmark", async function renderPerfEmpiricalTrainBenchmark(options = {}) {
  return renderSection(renderTrainGridSection, options);
});

export const renderPerfEmpiricalExpectedVsActual = resilientRender("renderPerfEmpiricalExpectedVsActual", async function renderPerfEmpiricalExpectedVsActual(options = {}) {
  return renderSection(renderExpectedVsActualSection, options);
});

export const renderPerfEmpiricalDeviceComparison = resilientRender("renderPerfEmpiricalDeviceComparison", async function renderPerfEmpiricalDeviceComparison(options = {}) {
  return renderSection(renderDeviceComparisonSection, options);
});

export const renderPerfEmpiricalTrainingCurves = resilientRender("renderPerfEmpiricalTrainingCurves", async function renderPerfEmpiricalTrainingCurves(options = {}) {
  return renderSection(renderTrainingCurvesSection, options);
});

export const renderPerfEmpiricalDiagnostics = resilientRender("renderPerfEmpiricalDiagnostics", async function renderPerfEmpiricalDiagnostics(options = {}) {
...
export const renderPerfEmpiricalModelSelection = resilientRender("renderPerfEmpiricalModelSelection", async function renderPerfEmpiricalModelSelection() {
  const root = el("div");
  root.style.display = "grid";
...
export const renderPerfEmpirical = resilientRender("renderPerfEmpirical", async function renderPerfEmpirical(options = {}) {
  const root = el("section");
