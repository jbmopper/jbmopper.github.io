...
// Every history column the row mappings below read; the files carry ~30.
const SWEEP_HISTORY_COLUMNS = [
  "run_name", "config.run_name", "_step", "step", "LR", "config.scheduler_lr_max", "lr_max", "Loss",
  "Eval Loss", "Eval/Loss", "Eval Perplexity", "Eval/Perplexity", "Throughput/Tokens per sec",
  "Time/Total step", "Grad/Norm (clipped)", "Grad/Norm (unclipped)", "state"
];
...
const OPTIMIZER_SWEEP_HISTORY_COLUMNS = [
  "run_name", "config.run_name", "name", "run_id", "_step", "step", "LR", "config.scheduler_lr_max", "lr_max",
  "Loss", "Eval Loss", "Eval/Loss", "Eval Best loss", "Eval/Best loss", "Sweep Metric M",
  "Grad/Norm (clipped)", "Grad/Norm (unclipped)", "Throughput/Tokens per sec", "Time/Total step"
];
  ...
  if (!sweepDataPromise) {
    sweepDataPromise = (async () => {
      const [mainTable, historyTable] = await Promise.all([
        readParquet(ATTACHMENTS.main),
        readParquet(ATTACHMENTS.history, {columns: SWEEP_HISTORY_COLUMNS})
      ]);
...
async function loadOptimizerSweepData() {
  if (!optimizerSweepDataPromise) {
    optimizerSweepDataPromise = (async () => {
      const [mainTable, historyTable] = await Promise.all([
        readParquet(OPTIMIZER_SWEEP_ATTACHMENTS.main),
        readParquet(OPTIMIZER_SWEEP_ATTACHMENTS.history, {columns: OPTIMIZER_SWEEP_HISTORY_COLUMNS})
      ]);
...
export const renderOptimizerSweepPaths = resilientRender("renderOptimizerSweepPaths", async function renderOptimizerSweepPaths(options = {}) {
...
export const renderOptimizerSweepClusters = resilientRender("renderOptimizerSweepClusters", async function renderOptimizerSweepClusters(options = {}) {
  const root = el("section");
...
// The optimizer-sweep page shows six sections of one analysis in separate cells.
...
async function buildOptimizerSweepEvalLoss() {
  ...
  const evalPanel = card();
  evalPanel.dataset.section = "optimizer-window-panel";
  evalPanel.append(controlsRow, summaryHost, evalHost);

  const gradSummaryHost = el("p");
  gradSummaryHost.style.margin = "0";
  const gradHost = el("div");
  gradHost.dataset.section = "optimizer-grad-norm-chart";
  const gradPanel = card();
  gradPanel.dataset.section = "optimizer-grad-panel";
  gradPanel.append(gradSummaryHost, gradHost);
  ...
  function renderMuQuadrantAnalysis() {
    ...
    const chartNode = Plot.plot({
      onReady: (instance) => bindZoomListener(instance),
    ...
    // Charts mount lazily, when scrolled near; bind once this one exists.
    function bindZoomListener(instance) {
  ...
  const evalRows = enrichedRows.filter((row) => Number.isFinite(row.eval_loss) && row.eval_loss > 0);
  const gradRows = enrichedRows.filter((row) => Number.isFinite(row.grad_unclipped) && row.grad_unclipped > 0);
  const summaryText = `${runNames.length.toLocaleString("en-US")} runs total, ${evalRows.length.toLocaleString(
    "en-US"
  ...
  const sections = new Map([
    ["optimizer-window-panel", [evalPanel, renderEvalChart]],
    ["optimizer-grad-panel", [gradPanel, renderGradChart]],
    ["optimizer-rank-correlation-table", [muTableHost, renderMuCheckpointTables]],
    ["optimizer-regression-coefficient-table", [muRegressionHost, renderMuRegressionChart]],
    ["optimizer-logistic-regression-table", [muLogisticHost, renderMuLogisticRegression]],
    ["optimizer-quadrant-analysis", [muQuadrantHost, renderMuQuadrantAnalysis]]
  ]);
  const rendered = new Set();
  return {
    names: Array.from(sections.keys()),
    section(name) {
...
let sharedOptimizerEvalLoss = null;

function optimizerEvalLoss() {
...
export const renderOptimizerSweepEvalLoss = resilientRender("renderOptimizerSweepEvalLoss", async function renderOptimizerSweepEvalLoss() {
  const root = el("section");
...
export const renderOptimizerSweepEvalLossChart = resilientRender("renderOptimizerSweepEvalLossChart", async function renderOptimizerSweepEvalLossChart() {
  return optimizerSection("optimizer-window-panel", "Eval-loss chart section is unavailable.");
});

export const renderOptimizerSweepGradNormChart = resilientRender("renderOptimizerSweepGradNormChart", async function renderOptimizerSweepGradNormChart() {
  return optimizerSection("optimizer-grad-panel", "Grad-norm chart section is unavailable.");
});

export const renderOptimizerSweepRankCorrelationTable = resilientRender("renderOptimizerSweepRankCorrelationTable", async function renderOptimizerSweepRankCorrelationTable() {
  return optimizerSection("optimizer-rank-correlation-table", "Rank-correlation table section is unavailable.");
});

export const renderOptimizerSweepRegressionCoefficients = resilientRender("renderOptimizerSweepRegressionCoefficients", async function renderOptimizerSweepRegressionCoefficients() {
  return optimizerSection(
    "optimizer-regression-coefficient-table",
    "Regression-coefficient section is unavailable."
  );
});

export const renderOptimizerSweepLogisticTable = resilientRender("renderOptimizerSweepLogisticTable", async function renderOptimizerSweepLogisticTable() {
  return optimizerSection("optimizer-logistic-regression-table", "Logistic-regression table section is unavailable.");
});

export const renderOptimizerSweepQuadrantAnalysis = resilientRender("renderOptimizerSweepQuadrantAnalysis", async function renderOptimizerSweepQuadrantAnalysis() {
  return optimizerSection("optimizer-quadrant-analysis", "Quadrant analysis section is unavailable.");
});

export const renderOptimizerSweepRunEvalLoss = resilientRender("renderOptimizerSweepRunEvalLoss", async function renderOptimizerSweepRunEvalLoss(options = {}) {
  const root = el("section");
...
export const renderLrSweepLrGradScatter = resilientRender("renderLrSweepLrGradScatter", async function renderLrSweepLrGradScatter(options = {}) {
  const root = el("section");
...
export const renderLrSweepLrClipHistogram = resilientRender("renderLrSweepLrClipHistogram", async function renderLrSweepLrClipHistogram(options = {}) {
  const root = el("section");
...
export const renderLrSweepSummaryTable = resilientRender("renderLrSweepSummaryTable", async function renderLrSweepSummaryTable(options = {}) {
  const root = el("section");
...
export const renderLrSweep = resilientRender("renderLrSweep", async function renderLrSweep(options = {}) {
  const root = el("section");
