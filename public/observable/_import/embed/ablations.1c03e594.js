...
// Every history column the row mapping below reads; the file carries 32.
const ABLATION_HISTORY_COLUMNS = [
  "run_name", "config.run_name", "run_label", "config.run_label", "config.model_settings.norm_mode", "norm_mode",
  "config.model_settings.use_rope", "use_rope", "config.model_settings.ffn_type", "ffn_type", "_step", "step",
  "Loss", "Eval Loss", "Eval/Loss", "Eval Perplexity", "Eval/Perplexity", "Grad/Norm (unclipped)",
  "Grad/Norm (Unclipped)", "Grad/Norm (raw)", "grad_norm_unclipped", "Throughput/Tokens per sec", "Time/Total step"
];
  ...
  if (!ablationDataPromise) {
    ablationDataPromise = (async () => {
      const [mainTable, historyTable] = await Promise.all([
        ...
        readParquet(ATTACHMENTS.history, {columns: ABLATION_HISTORY_COLUMNS})
      ]);
...
export const renderAblationsByRun = resilientRender("renderAblationsByRun", async function renderAblationsByRun(options = {}) {
...
export const renderAblationsSummary = resilientRender("renderAblationsSummary", async function renderAblationsSummary(options = {}) {
  return renderAblationsByRun({...options, showPerRun: false});
});

export const renderAblationsOverviewCharts = resilientRender("renderAblationsOverviewCharts", async function renderAblationsOverviewCharts(options = {}) {
  return renderAblationsByRun({...options, showPerRun: false});
});

export const renderAblationsHistory = resilientRender("renderAblationsHistory", async function renderAblationsHistory(options = {}) {
  return renderAblationsByRun({...options, showOverall: false});
});

export const renderAblationSection = resilientRender("renderAblationSection", async function renderAblationSection(options = {}) {
  return renderAblationsByRun({...options, showOverall: false, showHeading: false});
});

export const renderAblations = resilientRender("renderAblations", async function renderAblations(options = {}) {
