...
export async function loadTraceRowsSummary(traceName) {
  if (summaryCache.has(traceName)) return summaryCache.get(traceName);
  ...
  const loadPromise = (async () => {
    ...
    summaryCache.set(traceName, rows);
    return rows;
  })();
...
export async function loadTraceRowsDetailed(traceName) {
  if (detailedCache.has(traceName)) return detailedCache.get(traceName);
  if (detailedLoadPromises.has(traceName)) return detailedLoadPromises.get(traceName);

  const loadPromise = (async () => {
    const table = await loadParquetTable(traceName);
    const rows = Array.from(table, (row, idx) => {
    ...
    }).filter((row) => Number.isFinite(row.start_ns) && Number.isFinite(row.end_ns));

    detailedCache.set(traceName, rows);
    return rows;
  })();
